...
def generate_story_html(target: str | None = None, embed_images: bool = False) -> str:
    ...
    owner = resolved_owner_profile(settings)
    display_name = html.escape(owner["display_name"])
    telescope_model = html.escape(owner["telescope_model"])
    location_label = html.escape(owner["location_label"])
    header_title = f"{display_name}'s Astrophotography Journey"
    ...
    subtitle_parts = [f"Since {_fmt_date(_FIRST_LIGHT_DATE)}", telescope_model]
    if owner["show_location_publicly"] and location_label:
        subtitle_parts.append(location_label)
...
<div class="export-bar">
  Generated by SeeStar Database · {telescope_model} · Claude AI
...
<nav style="position:sticky;top:0;z-index:100;background:var(--bg);
  ...
  <a id="seestar-dashboard" href="/"
     style="white-space:nowrap;flex-shrink:0;color:var(--text2)">&#8592; Dashboard</a>
  <script>document.getElementById('seestar-dashboard').href='http://'+location.hostname+'/';</script>
  <a href="/">Home</a>
